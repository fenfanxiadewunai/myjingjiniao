...
public abstract class SocketIOWithTimeout {
	private static final Log LOG = LogFactory.getLog(SocketIOWithTimeout.class);
	...
	private static class SelectorPool{
		...
		private void trimIdleSelectors(long now){
			...
			for(ProviderInfo pList = providerList;pList!=null;pList = pList.next){
				...
				for(Iterator<SelectorInfo> iter = pList.queue.iterator();iter.hasNext();){
					SelectorInfo info = iter.next();
